...
export function registerSyncHandlers(mainWindow: BrowserWindow): void {
  ipcMain.handle('sync:detectChanges', (_event, albumId: string) => {
    ...
    try {
      ...
      // Detect changes
      const changes = {
        new: [] as Array<{ filename: string; size: number; mtime: string; width?: number; height?: number; hash?: string }>,
        ...
        renamed: [] as Array<{ id: number; serverId: number | null; oldFilename: string; newFilename: string; localFilePath: string }>,
